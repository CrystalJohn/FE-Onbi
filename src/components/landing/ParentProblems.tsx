...
import { MessageSquareX, Timer, Compass, Sparkles } from 'lucide-react';
        ...
        {problems.map((prob, idx) => {
          ...
          return (
            <motion.div
              ...
              className={`p-6 md:p-8 bg-white/45 bg-white/70 rounded-3xl border border-white/60 shadow-2xs hover:shadow-xs flex flex-col justify-between transition-all group overflow-hidden relative ${prob.glow}`}
            >
              ...
              <div className="space-y-5 relative z-10">
                ...
                <div className="space-y-2">
                  ...
                  <h3 className="font-display text-lg font-bold text-slate-950 tracking-tight leading-tight pt-1">
                    {prob.title}
                  </h3>
                  <p className="text-xs text-[#78756f] leading-relaxed font-medium pt-1.5">
                    {prob.description}
                  </p>
                </div>
              </div>

              <div className="mt-6 pt-4 border-t border-[#ccc9bf]/20 text-[9px] font-mono text-[#b0ada6] uppercase tracking-wider font-bold">
                Solitary Desk Constraint
              </div>
            </motion.div>
          );
        })}
