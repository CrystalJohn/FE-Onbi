...
function LogoutConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative z-10 w-full max-w-sm rounded-[28px] border border-white/80 bg-white/95 p-6 shadow-[0_28px_80px_rgba(15,23,42,0.22)] backdrop-blur-xl animate-in fade-in zoom-in-95 duration-200">
        {/* Icon */}
        <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-red-50 mx-auto">
          <AlertTriangle className="h-6 w-6 text-red-500" />
        </div>

        {/* Content */}
        <div className="text-center">
          <h2 className="text-base font-bold text-slate-900">Bạn muốn đăng xuất?</h2>
          <p className="mt-1.5 text-sm text-slate-500 leading-relaxed">
            Hẹn gặp lại! Bạn có thể đăng nhập lại bất cứ lúc nào để tiếp tục quan sát bé.
          </p>
        </div>
...
export function Sidebar() {
  ...
  const NavLinks = ({ onClickLink }: { onClickLink?: () => void }) => (
    <nav aria-label="Điều hướng chính" className="mt-5 flex-1 space-y-1.5">
      {nav.map((item) => {
        ...
        return (
          <Link
            ...
            className={`flex min-h-12 items-center gap-3 overflow-hidden rounded-2xl px-3.5 text-sm font-semibold transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 focus-visible:ring-offset-2 ${
              isActive
  ...
  const LogoutButton = () => (
    <button
      ...
      className="flex w-full min-h-11 items-center gap-2.5 rounded-2xl px-3.5 text-sm font-semibold text-red-500 transition-colors hover:bg-red-50 hover:text-red-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400"
    >
  ...
  return (
    <>
      {/* Logout confirm modal — rendered at root level */}
      {showLogoutModal && (
        <LogoutConfirmModal
      ...
      {/* Desktop sidebar */}
      <aside className="sticky top-0 z-30 hidden h-dvh w-[240px] shrink-0 p-3 lg:block">
        <div className="relative flex h-[calc(100dvh-24px)] flex-col overflow-hidden rounded-[32px] border border-white/80 bg-white/75 p-2.5 shadow-[0_24px_70px_rgba(15,23,42,0.10)] backdrop-blur-xl">
          ...
          <div className="mt-3 space-y-1 border-t border-slate-100 pt-3">
            ...
            <div className="flex min-h-10 items-center gap-2 rounded-full px-3.5 text-slate-400">
              ...
              <span className="text-xs font-medium">Phiên bản v1.0.0</span>
            </div>
          </div>
        </div>
      </aside>
      ...
      {isMobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <aside className="absolute inset-y-3 left-3 flex w-[min(240px,calc(100vw-24px))] flex-col rounded-[32px] border border-white/80 bg-white/90 p-4 shadow-[0_28px_80px_rgba(15,23,42,0.22)] backdrop-blur-xl">
