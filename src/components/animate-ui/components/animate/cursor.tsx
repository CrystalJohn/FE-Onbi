...
function Cursor({ className, ...props }: CursorProps) {
  return (
    <CursorPrimitive asChild {...props}>
      <svg
        className={cn('size-6 text-slate-800 drop-shadow-md', className)}
...
function CursorFollow({
  ...
  return (
    <CursorFollowPrimitive asChild sideOffset={sideOffset} alignOffset={alignOffset} {...props}>
      <div
        className={cn(
          'bg-slate-800 rounded-md text-white px-2 py-1 text-sm drop-shadow-md',
          className,
        )}
      >
        {children}
