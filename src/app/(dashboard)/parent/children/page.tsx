...
import { Bot, CalendarDays, Lock, Pencil, Plus, Radio, Settings2, Trash2 } from 'lucide-react';
import { api } from '@/lib/api';
import { ageFromBirthDate as age } from '@/lib/format';
...
export default function ChildrenListPage() {
  ...
  if (loading) return <div className="mx-auto max-w-7xl animate-pulse space-y-5"><div className="h-14 w-72 rounded-xl bg-slate-200" /><div className="grid gap-5 lg:grid-cols-2"><div className="h-96 rounded-3xl bg-slate-200" /><div className="h-96 rounded-3xl bg-slate-200" /></div></div>;
  ...
  return <div className="mx-auto max-w-7xl space-y-5">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="flex flex-col-reverse gap-2 sm:flex-row sm:items-center">
        ...
        <button onClick={() => setShowForm(true)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-[#0B008B] px-5 text-sm font-bold text-white shadow-[0_10px_24px_rgba(11,0,139,0.20)] transition-colors duration-200 hover:bg-[#08006D] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0B008B] focus-visible:ring-offset-2">
          <Plus aria-hidden="true" className="h-4 w-4" />Thêm hồ sơ bé
        </button>
      </div>
    </header>
    ...
    <AddChildModal open={showForm} onClose={() => setShowForm(false)} onCreated={loadChildren} />

    {children.length === 0
      ? <section className="rounded-[32px] border border-white/80 bg-white/70 px-6 py-10 text-center shadow-[0_24px_70px_rgba(15,23,42,0.08)] backdrop-blur-xl"><h2 className="text-lg font-bold text-slate-900">Chưa có hồ sơ trẻ</h2><p className="mt-2 text-sm text-slate-500">Thêm hồ sơ đầu tiên để bắt đầu kết nối thiết bị ONBI.</p></section>
      : <section aria-label="Danh sách hồ sơ trẻ" className="grid gap-5 lg:grid-cols-2 2xl:grid-cols-3">{children.map((child) => <StudentCard key={child.id} child={child} onDelete={() => setConfirmChild(child)} />)}</section>
    }
...
function StudentCard({ child, onDelete }: { child: ChildHub; onDelete: () => void }) {
  ...
  return (
    <article className="group relative flex h-full w-full flex-col overflow-visible rounded-[28px] border border-white/80 bg-gradient-to-b from-white via-white/95 to-cyan-50/80 shadow-[0_20px_60px_rgba(15,23,42,0.09)]">
