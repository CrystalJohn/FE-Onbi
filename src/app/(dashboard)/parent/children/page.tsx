...
import { Bot, CalendarDays, History, Images, MoreHorizontal, Pencil, Plus, Radio, Settings2, Timer, Trash2 } from 'lucide-react';
...
export default function ChildrenListPage() {
  ...
  useEffect(() => {
    ...
    const closeOnOutsideClick = (event: PointerEvent) => {
      ...
      if (!target.closest('[data-child-menu]')) setOpenMenuId(null);
    };
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="flex flex-col-reverse gap-2 sm:flex-row sm:items-center">
        ...
        <button onClick={() => setShowForm((value) => !value)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-[#0B008B] px-5 text-sm font-bold text-white shadow-[0_10px_24px_rgba(11,0,139,0.20)] transition-colors duration-200 hover:bg-[#08006D] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0B008B] focus-visible:ring-offset-2">
          {!showForm && <Plus aria-hidden="true" className="h-4 w-4" />}{showForm ? 'Đóng biểu mẫu' : 'Thêm học sinh'}
        </button>
      </div>
    </header>
    ...
    {children.length === 0 ? <section className="rounded-[32px] border border-white/80 bg-white/70 px-6 py-16 text-center shadow-[0_24px_70px_rgba(15,23,42,0.08)] backdrop-blur-xl"><h2 className="text-lg font-bold text-slate-900">Chưa có hồ sơ trẻ</h2><p className="mt-2 text-sm text-slate-500">Thêm hồ sơ đầu tiên để bắt đầu kết nối thiết bị ONBI.</p></section> : <section aria-label="Danh sách hồ sơ trẻ" className="grid items-start gap-6 xl:grid-cols-2">{children.map((child) => <StudentCard key={child.id} child={child} isMenuOpen={openMenuId === child.id} onToggleMenu={() => setOpenMenuId((current) => current === child.id ? null : child.id)} onCloseMenu={() => setOpenMenuId(null)} onDelete={() => void deleteChild(child)} />)}</section>}
  </div>;
}

function StudentCard({ child, isMenuOpen, onToggleMenu, onCloseMenu, onDelete }: { child: ChildHub; isMenuOpen: boolean; onToggleMenu: () => void; onCloseMenu: () => void; onDelete: () => void }) {
  const monitoring = child.session?.status === 'active';
  return <article className="group relative w-full max-w-[560px] overflow-visible rounded-[28px] border border-white/80 bg-gradient-to-b from-white via-white/95 to-cyan-50/80 shadow-[0_20px_60px_rgba(15,23,42,0.09)]">
    ...
    <div className="relative z-10 p-5">
      <div className="flex items-start gap-4">
        ...
        <div className="relative shrink-0" data-child-menu>
          <button type="button" onClick={onToggleMenu} className="grid h-11 w-11 place-items-center rounded-full border border-slate-200/80 bg-white/75 text-slate-600 shadow-sm transition-colors hover:border-cyan-200 hover:bg-cyan-50 hover:text-cyan-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500" aria-label={`Mở menu thao tác cho ${child.name}`} aria-expanded={isMenuOpen} aria-haspopup="menu">
            <MoreHorizontal aria-hidden="true" className="h-5 w-5" />
          </button>
          {isMenuOpen && (
            <div role="menu" className="absolute right-0 top-12 z-50 w-52 origin-top-right animate-in overflow-hidden rounded-2xl border border-white/80 bg-white/90 p-1.5 shadow-[0_18px_50px_rgba(15,23,42,0.16)] backdrop-blur-xl duration-150 fade-in slide-in-from-top-1 zoom-in-95 motion-reduce:animate-none">
              <Link role="menuitem" onClick={onCloseMenu} href={`/parent/children/${child.id}`} className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-100 focus:bg-slate-100 focus:outline-none"><Pencil aria-hidden="true" className="h-4 w-4 text-slate-500" />Chỉnh sửa</Link>
              <Link role="menuitem" onClick={onCloseMenu} href={`/parent/monitoring/${child.id}/history`} className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-100 focus:bg-slate-100 focus:outline-none"><History aria-hidden="true" className="h-4 w-4 text-slate-500" />Lịch sử</Link>
              <Link role="menuitem" onClick={onCloseMenu} href={`/parent/monitoring/${child.id}/pomodoro`} className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-100 focus:bg-slate-100 focus:outline-none"><Timer aria-hidden="true" className="h-4 w-4 text-slate-500" />Pomodoro</Link>
              <Link role="menuitem" onClick={onCloseMenu} href={`/parent/monitoring/${child.id}/snapshots`} className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-100 focus:bg-slate-100 focus:outline-none"><Images aria-hidden="true" className="h-4 w-4 text-slate-500" />Ảnh cảnh báo</Link>
              <div className="my-1 border-t border-slate-100" />
              <button role="menuitem" type="button" onClick={() => { onCloseMenu(); onDelete(); }} className="flex min-h-10 w-full items-center gap-3 rounded-xl px-3 text-left text-sm font-medium text-red-600 transition-colors hover:bg-red-50 focus:bg-red-50 focus:outline-none"><Trash2 aria-hidden="true" className="h-4 w-4" />Xóa hồ sơ</button>
            </div>
          )}
        </div>
      </div>
