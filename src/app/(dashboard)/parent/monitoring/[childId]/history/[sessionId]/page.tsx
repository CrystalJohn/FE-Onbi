...
import { AlertTriangle, Camera, Clock3, ImageIcon } from 'lucide-react';
import BackButton from '@/components/ui/BackButton';
import { api } from '@/lib/api';
import { formatDurationSec, formatSmartTime } from '@/lib/format';
...
export default function SessionDetailPage({ params }: { params: Promise<{ childId: string; sessionId: string }> }) {
  ...
  return <div className="mx-auto max-w-5xl space-y-5">
    <header className="flex items-center gap-3"><BackButton fallback={`/parent/monitoring/${childId}/history`} /><div><p className="text-sm font-medium text-cyan-700">Chi tiết phiên</p><h1 className="text-2xl font-bold text-slate-950">{session ? formatDate(session.startedAt) : `#${sessionId}`}</h1></div></header>
    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_1.4fr]"><section className="rounded-3xl border border-slate-200 bg-white p-5"><div className="mb-5 flex items-center gap-3"><Clock3 className="h-5 w-5 text-cyan-600" /><h2 className="font-bold text-slate-900">Các chu kỳ học</h2></div>{studies.length === 0 ? <Empty icon={Clock3} text="Phiên này chưa có dữ liệu Pomodoro." /> : <div className="space-y-3">{studies.map((item) => <article key={item.id} className={`rounded-2xl border p-4 ${scoreCardClass(item.score)}`}><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-slate-800">Chu kỳ {item.cycleNumber}</p><p className="mt-1 text-xs text-slate-500">{formatDate(item.startedAt)}</p></div><div className="flex flex-wrap items-center justify-end gap-2">{item.score != null && <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${scoreBadgeClass(item.score)}`}>{Math.round(item.score)}% {scoreLabel(item.score)}</span>}<span className="rounded-full bg-cyan-100 px-2.5 py-1 text-xs font-semibold text-cyan-700">{statusLabel(item.status)}</span></div></div><div className="mt-3 flex gap-5 text-sm text-slate-600"><span>Học: <b>{formatSeconds(item.actualStudySeconds)}</b></span><span>Nghỉ: <b>{formatSeconds(item.actualBreakSeconds)}</b></span></div></article>)}</div>}</section><section className="rounded-3xl border border-slate-200 bg-white p-5"><h2 className="mb-4 font-bold text-slate-900">Ảnh cảnh báo</h2>{snapshots.length === 0 ? <Empty icon={ImageIcon} text="Không có ảnh cảnh báo trong phiên này." /> : <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{snapshots.map((snapshot) => <figure key={snapshot.id} className="overflow-hidden rounded-2xl border border-slate-100 bg-slate-50"><div className="aspect-[4/3] bg-slate-100">{snapshot.imageUrl ? <img src={toImageUrl(snapshot.imageUrl)} alt={snapshot.description ?? 'Ảnh cảnh báo'} className="h-full w-full object-cover" /> : <Camera className="m-auto h-full w-8 text-slate-300" />}</div><figcaption className="p-3"><p className="truncate text-xs font-semibold text-slate-700">{snapshot.description ?? typeLabel(snapshot.type)}</p><p className="mt-1 text-[11px] text-slate-400">{formatDate(snapshot.capturedAt)}</p></figcaption></figure>)}</div>}</section></div>
  </div>;
}
...
function formatSeconds(value: number) { return formatDurationSec(value ?? 0); }
